...
<li class="<?php bp_activity_css_class(); ?>" id="activity-<?php bp_activity_id(); ?>" data-bp-activity-id="<?php bp_activity_id(); ?>" data-bp-timestamp="<?php bp_nouveau_activity_timestamp(); ?>" data-bp-activity="<?php ( function_exists('bp_nouveau_edit_activity_data') ) ? bp_nouveau_edit_activity_data() : ''; ?>">
	...
	<div class="bp-activity-head">
		<div class="activity-avatar item-avatar">
			<?php
			if( 'followed' == bp_get_activity_type() || 'unfollowed' == bp_get_activity_type() ) {
				$person_link = bp_core_get_userlink(bp_get_activity_secondary_item_id(), $just_link = true );
				$person_avatar = bp_core_fetch_avatar( array( 'item_id' => bp_get_activity_secondary_item_id(), 'type'   => 'thumb', 'width'  => '40', 'height' => '40'));
			} else {
				$person_link = bp_get_activity_user_link();
				$person_avatar = bp_get_activity_avatar( array( 'type' => 'full' ) );
			}
			?>
			<a href="<?php echo $person_link; ?>"><?php echo $person_avatar; ?></a>
		</div>
